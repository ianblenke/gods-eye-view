...
6. **Ledger.** Each smaller ledger entry must agree with a real test in the change. The gate allows a small loss of coverage in a band file (see the requirement "Count band for band files"). Thus also check each removed or changed test and each changed code file that a band file uses. Each history line must name this change. Report a larger entry, a new entry and an entry that the change removed for a file that is still there.
