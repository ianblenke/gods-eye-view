...
The places the owner wants are feature entities: the stations of a radio mesh. Each station is a feature of interest with a `Point`. One system hosts each feature, through a link. A client that reads only `/systems` never sees a feature.
