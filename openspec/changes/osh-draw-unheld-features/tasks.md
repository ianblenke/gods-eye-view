...
- [x] 1.1 Write `specs/osh/spec.md`: `osh-058`, `osh-059`, `osh-060` ADDED; `osh-042`, `osh-057` and `osh-032` MODIFIED; the rest carried.
...
- [x] 1.3 Write `design.md` with D52 to D58.
...
- [x] 2.1 Keep the name of each `[osh-042]` test that the file has now.
...
- [x] 2.2 Write the `[osh-058]` tests in `src/data/oshSystems.test.mjs`. Use `foi-fixture-`, `sys-fixture-` and `ds-fixture-` ids.
...
- [x] 3.1b Write the `[osh-032]` layer test for a held feature that a stream moves.
  - `[osh-032]` the detail for a selected held feature that a stream moves shows its name and Placed by with the location's age.
- [x] 3.2 Extend the test `[osh-057]` a fresh location that names a feature moves the feature entity and never places a system.
...
- [x] 3.3 Write the `[osh-060]` test: four refreshes on one layer.
...
- [x] 4.1 Write the `[osh-032]` detail tests in `src/layers/osh/detail.test.mjs`.
