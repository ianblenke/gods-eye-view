...
- [ ] 1.1 Write `specs/osh/spec.md`: `osh-058`, `osh-059`, `osh-060` ADDED; `osh-042`, `osh-057` MODIFIED; the rest carried.
  ...
  - `osh-location-streams` is already archived. The base was checked against the archived spec: every carried scenario is byte-identical, and `osh-032`, `osh-042` and `osh-057` differ only by the clauses this change adds.
