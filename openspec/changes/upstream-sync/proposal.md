...
In this text, the owner of the project is the person who accepts this change on `main`. Rule 21 of `AGENTS.md` calls this person "the person who merges a change". The author of this change is the agent that wrote it.
...
- Keep the credential boundary in the new search code of the upstream project. The HTTP geospatial provider, and the first use of `createGoogleGeocoder` in `src/search/defaults.js`, call the server route `/api/google/geocode`, with no key in the request. The browser search code has no `resolveApiKey` input.
...
- Gaps that this change closes: the tests of the upstream project cover code that the fork left uncovered. The ratchet command recorded 16 entries as closed. Examples are `src/data/radio.js` (511 lines), `src/annotations/annotationResolver.js` (562 lines) and `src/data/telegeographySubmarineCables.js` (66 lines). It also recorded 3 code files and 1 test file as removed with their entries, and smaller counts for 8 files. Two of these files, `src/data/labelArbiter.js` and `src/data/lifecycle.js`, are counts that change between runs (`sync-counts-change-between-runs`). The history of the ledger has each of these lines with the change name `upstream-sync`.
...
- `sync-files-edited-after-the-merge`: Three more code files have an adopt line. The fork did not change them before this change, only the upstream project did, and a person then changed them in this change. These are `src/app/constructCatalog.js` (3 lines, 4 branches and 1 function), `src/search/defaults.js` (2 branches) and `src/search/http.js` (4 branches and 2 functions). The author of this change checked the uncovered items: they are upstream code, and the code of this change in these files is covered. The person who accepts this change on `main` reads the Git diff between the merged commit and HEAD for these three files too.
- `sync-two-upstream-tests-edited`: The change edits two upstream tests without a scenario. A later change of these two tests in the upstream project can conflict with these edits, when the fork merges that change. The person who resolves the conflict keeps the fix.
...
- `sync-counts-change-between-runs`: Some files have a not-covered count that changes between runs, probably because a test runs different lines when its events happen at different times. Four examples are `server/providers/vessels/ais-store.js` (40 or 44 lines), `src/data/lifecycle.js` (177 to 184 lines), `src/layers/wind/rendering.js` (a total that changes by 1) and `src/data/labelArbiter.js` (2 branches). The gates of this change compare the count of a changed file exactly. So a run can stop with `LEDGER-LARGER-GAP`, `LEDGER-LOST-COVERAGE` or `LEDGER-STALE` for such a file. The gates of the push to `main` use the tolerance, and the gates of a pull request do not.
- `sync-hand-written-adopt-lines`: The author of this change wrote two adopt lines by hand, for the first two examples of `sync-counts-change-between-runs`. They are the lines 1424 and 1425 of `openspec/trace/history.jsonl`, and they have the larger counts that the runs measured. The author also set the entry of `src/data/lifecycle.js` in `openspec/trace/gaps.json` to the larger count by hand. The next ratchet run then wrote the measured counts. The counts of the Impact are the counts of the 797 adopt lines that the command wrote. The person who accepts this change on `main` reads these two lines.
- `sync-remembered-statuses`: The HTTP geospatial provider remembers each Google status that comes with no place on an HTTP 200 answer. Only the status `ZERO_RESULTS` has a test. The owner decides if a status such as `REQUEST_DENIED` must not be remembered.
