...
- The 525 code files have 60497 lines, 5868 branches and 1683 functions that no test covers. Of these files, 17 have untrue coverage, because a test runs them through a transform. The 272 test files have 3413 tests without a scenario ID.
- Gaps that this change closes: the tests of the upstream project cover code that the fork left uncovered. The ratchet command recorded entries as closed, as removed with their file, and as smaller. The history of the ledger has each of these lines with the change name `upstream-sync`.
