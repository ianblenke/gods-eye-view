...
- [x] 2.8 Keep the tests for `osh-028`, `osh-029`, `osh-030`, `osh-031`, `osh-032`, `osh-046`.
- [x] 2.9 Keep the tests for `osh-049`, `osh-057`, `osh-086`, `osh-087`, `osh-088`.
- [x] 2.10 Keep the tests for `osh-089`, `osh-093`, `credential-boundary-013`, `credential-boundary-015`.

## 3. The code

- [x] 3.1 Merge the upstream branch.
- [x] 3.2 Resolve the conflicts in the files that both sides changed.
- [x] 3.3 Move the OSH panel to the template.
...
- [x] 3.11 Clear the timer in the helper `within` of `src/sdr/controller.test.mjs`.
- [x] 3.12 Raise the ceiling of the ingestion test in `server/providers/transitHistory.test.mjs` to ten seconds.
- [x] 3.13 Restore the upstream test of the ALPR layer in `src/voice/gevActions.test.mjs`.
- [x] 3.14 Correct the text of `docs/CURRENT-STATE.md` about the search and the reverse lookup.
