...
- [ ] 2.6 Add `src/layers/osh/hosts.js` and `src/app/layers/osh.js` to the address scan of `[osh-034]`, and `src/sources/httpBody.js` to the scan of `[osh-005]`.
...
- [ ] 2.7 Keep the tests for `osh-028`, `osh-029`, `osh-030`, `osh-031`, `osh-032`, `osh-046`.
- [ ] 2.8 Keep the tests for `osh-049`, `osh-057`, `osh-086`, `osh-087`, `osh-088`.
- [ ] 2.9 Keep the tests for `osh-089`, `osh-093`, `credential-boundary-013`, `credential-boundary-015`.

## 3. The code

- [ ] 3.1 Merge the upstream branch.
- [ ] 3.2 Resolve the conflicts of the 32 files that both sides changed.
- [ ] 3.3 Move the OSH panel to the template, and give the OSH layer the token `3`.
- [ ] 3.4 Write `src/app/layers/osh.js`, and place the layer in the catalog until the tests of `osh-095` and `osh-096` pass.
- [ ] 3.5 Write the server route calls in `src/search/http.js` and `src/search/defaults.js` until the tests of `credential-boundary-014` pass.
- [ ] 3.6 Format the OSH files with the Prettier format of the upstream project.
- [ ] 3.7 List the OSH modules in `scripts/package-boundaries.json`.
- [ ] 3.8 Move `createOshPanelHosts` to `src/layers/osh/hosts.js`.
- [ ] 3.9 Clear the timer in the helper `within` of `src/sdr/controller.test.mjs`.
- [ ] 3.10 Raise the ceiling of the ingestion test in `server/providers/transitHistory.test.mjs` to ten seconds.
- [ ] 3.11 Restore the upstream test of the ALPR layer in `src/voice/gevActions.test.mjs`.
- [ ] 3.12 Correct the text of `docs/CURRENT-STATE.md` about the search and the reverse lookup.
