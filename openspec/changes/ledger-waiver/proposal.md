...
- Add the requirement "Coverage waiver" with the scenarios `gap-ledger-079` to `gap-ledger-085`.
...
- `gap-ledger`: the requirements "Ratchet rule", "Comparison with the base commit" and "Ratchet command" get the waived count in five scenarios. The new requirement "Coverage waiver" gets `gap-ledger-079` to `gap-ledger-085`.
...
- Gaps that this change opens: none. The two gate files are at 100% and stay at 100%.
- Gaps that this change closes: none.
- This change writes no waiver. The change `credential-boundary` writes its own waiver after this change merges. The design gives the steps.
...
- `waiver-needs-entry`: A waiver applies to a file with a ledger entry. A wrong count in a complete file or in a new file gives `LEDGER-NEW-COVERAGE-GAP`, and no waiver removes that error. The ledger gets no new entry after the init command, and this change keeps that rule.
