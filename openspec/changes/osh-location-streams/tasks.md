...
- [x] 1.3 Write `design.md` with D44 to D51 and the request shape.
...
- [x] 3.2 Write `readOshSchemaLocation()` until 3.1 is green.
...
- [x] 3.4 Rewrite `extractOshLocation(result, reader)`. Give `mapOshObservation()` its reader. Until 3.3 is green.
...
- [x] 3.6 Write `mapOshLocationPage()` until 3.5 is green.
- [x] 3.7 Write the merge-function tests: a not-fresh location dropped first, a fresh feature move by id and by uid.
  - Also cover a fresh system placement above a point, the newer of two winning, an unknown feature dropped, a null name.
  - Rules out: a merge that places a host from a feature-referencing record; one that lets a record that is not fresh place anything.
  - Also rules out: one that prefers a point over a fresh record.
- [x] 3.8 Change the merge function until 3.7 is green.
...
- [x] 4.2 Write the three fixed pairs and their constants in `ids.js` until 4.1 is green.
...
- [x] 4.4 Write `createOshSchemaCache()` and `createOshSystemCache()` until 4.3 is green.
...
- [x] 4.6 Write `createOshLocationsPass()` until 4.5 is green.
...
- [x] 4.9 Add the property lists, the locations route and the schema read to `osh.js` until 4.7 and 4.8 are green.
...
- [x] 5.1 Write the source tests: `getLocations()`, its key-required answer, its throw, the fields it forwards, its empty query.
  - Rules out: a getter that sends a query; one that drops the failed count.
- [x] 5.2 Add `getLocations()` to `source.js` until 5.1 is green.
...
- [x] 5.4 Write the stream-placement tests: placement and its stat, the label from the pass's own name, the raw-id fallback.
  - Also cover the placeholder kept outside the union, retirement when not fresh, the selected exception, the feature-move priority, the click and the poll.
  - Rules out: a layer that places from geometry only; one that labels every unheld system with its raw id.
  - Also rules out: one that keeps an entity once not fresh; one that unions the placeholder; one that drops a selected entity.
- [x] 5.5 Change `update()` and the record bookkeeping in `layers/osh/index.js` until 5.3 and 5.4 are green.
...
- [x] 6.3 Write the `[osh-032]` detail tests for the `Placed by` line: the name and the age, the em-dash fallback.
  ...
  - Change `renderSystemHeader()` in `detail.js` until they are green.
...
- [x] 6.5 Fix an age-drift bug this merge exposed.
...
- [x] 6.13 Close the one branch `make ratchet` found still uncovered in `src/layers/osh/index.js`.
  ...
  - Its stream is no longer fresh the same refresh. It must not also count as retired.
