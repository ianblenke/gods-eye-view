...
- [ ] 1.1 Write `specs/osh/spec.md`: `osh-005` MODIFIED, and `osh-063` to `osh-074` ADDED.
...
- [ ] 1.3 Write `design.md` with D64 to D71.
...
- [ ] 2.1 Change the test `[osh-005]` in `src/data/oshProxy.test.mjs`. Keep its name and its tag.
...
- [ ] 2.2 Add a `[osh-006]` test in `src/data/oshProxy.test.mjs`: a wrong method on `/api/osh/live` gives `405` and no socket.
- [ ] 2.3 Write the `[osh-064]` tests in `src/data/oshIds.test.mjs` for `liveUrl()` and `assertLiveUrl()`.
...
- [ ] 2.4 Write the tests of `osh-063`, `osh-065` to `osh-070` in the new file `src/data/oshLive.test.mjs`.
  ...
  - `[osh-069]` the events `open` and `down`. The delays 1, 2, 4, 8, 16 and 30 seconds, and the reset after 30 seconds open. The heartbeat every 20 seconds.
  - `[osh-070]` on a failure, no event, header or log line holds the URL, the user name or the password.
- [ ] 2.5 Add `liveUrl()` and `assertLiveUrl()` to `server/providers/osh/ids.js` until 2.3 passes.
- [ ] 2.6 Add `oshOpenStream()` to `server/providers/osh/get.js`. It sets `binaryType`, passes the headers, and never sends a frame.
- [ ] 2.7 Write `server/providers/osh/live.js` with `createOshLiveHub()` until 2.4 passes.
- [ ] 2.8 Add the route `/api/osh/live` to `server/providers/osh.js` with the checks of `osh-063`, until 2.1 to 2.4 pass.
- [ ] 2.9 Run each mutation below on the server. Report one line for each mutation: the id and the test that failed.
...
- [ ] 3.1 Write the `[osh-071]` tests in `src/layers/osh/source.test.mjs` with a fake `EventSource`.
...
- [ ] 3.2 Add `openLive()` to `src/layers/osh/source.js` until 3.1 passes.
- [ ] 3.3 Write the `[osh-072]`, `[osh-073]` and `[osh-074]` tests in `src/data/oshLayer.test.mjs`. Register `t.after(() => layer.destroy(viewer))` in each.
  ...
  - `[osh-073]` a live observation changes the detail block and the entity position for a fresh location. An observation that is ahead of the clock moves nothing. The layer reads no second observation for that datastream.
  - `[osh-074]` a datastream with no open stream, or with `down` or `unsupported`, is polled. A stream that opens again stops the poll.
  - Keep every test that exists now, with its name.
- [ ] 3.4 Change `src/layers/osh/index.js` until 3.3 passes. A source with no `openLive()` keeps the polling of today.
