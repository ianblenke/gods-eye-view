...
- Change the scenario `osh-004` to name the WebSocket. Change `osh-005` for the new call site and the new file count.
...
- The provider holds more open connections. The limits are eight upstream sockets in all, and sixteen clients for each datastream.
...
- `osh-live-stream-cap`: the layer starts at most three streams. Each stream holds one connection, and a browser keeps about six connections to one origin over HTTP/1.1. A system with more datastreams streams the first three and polls the others.
...
- `osh-live-one-way`: the provider only listens. It sends no command and no message frame, and the server accepts none. The close frame of a socket that the provider closes is its only frame.
...
- `osh-live-silent-upstream`: the hub has no timer for a silent upstream socket, because a datastream can stay silent for a long time. The event `down` comes only after a close or an error. While a stream is open and the layer holds an observation, the layer does not poll. A socket that no longer works therefore leaves the last observation in the detail, with the age that the hub sent.
- `osh-live-memory`: the hub receives a whole frame before it checks its size. The map of refused datastreams never shrinks. The provider ignores the result of a write to a client, so a slow client can make its buffer grow.
- `osh-live-no-retry-after-refusal`: an answer `503` makes the `EventSource` fail for good. The layer treats it as `down`, polls the datastream, and starts no new stream for the same selection. A second selection within two seconds of a full hub gets `live_busy` for each stream, and the layer polls.
