...
- F2 major: correct in its result. The first worker had changed the word "prior" to "earlier" in the name of one old test, because it believed that the STE lint refuses the word. The lead put the old word back, ran the lint (0 errors), and compared all 32 old names with `origin/main` after removing the tags. All 32 are the same.
- F3 major: the four files (`labelArbiter.js`, `lifecycle.js`, `wind/rendering.js`, `worldOverlay.js`) are not edited by this change, and no test of this change closes their gaps. The ratchet command writes the count that it measures, and these counts change between runs (see `sync-counts-change-between-runs` in `upstream-sync`). The lead did not edit the ledger by hand. The proposal now names the four files and the values in the known limit `cyclones-ratchet-noise`.
