...
- `ci-samples`: The CI job does not keep samples, and no tool in the project makes them from a CI run. For this ledger, the author made the samples of one CI run by hand. The CI job keeps its results for 7 days. A later CI run can give counts outside a range. The author must then make the samples of that run, add them, and run the stability command. That command also writes the high counts 141 and 91 again, so the author must write 139 and 88 by hand.
...
- `range-slack`: In a file with a range, a change can remove the coverage of some lines, branches or functions. The number that it removes can be at most the width of the range. The spec adversary checks each changed test for these files. For a changed file, the gate compares the counts with the low count of the range. Thus a change that edits `src/data/manager.js` or `src/layers/flights/enrichment.js` can get `LEDGER-LARGER-GAP`. The author must then make the tests of that file give the same counts in each run.
- `unchanged-band`: The gate allows a band of 5, or 2% of the count when that is more, for the counts of a band file. Thus a change can remove this much coverage from a band file with removed tests, removed assertions or changed code that the file uses. For a band file with a range, the band adds to the high count. The ledger keeps the old count of a smaller gap until the next ratchet run. For a band file with no range, the ratchet command does not write a smaller covered count to the ledger, also for a smaller gap. The spec adversary checks each removed or changed test and each changed code file that a band file uses.
- `range-band-ratchet`: This limit is for a band file with a range and a not-covered count above its high count. The ratchet command then keeps that high count and writes the current total counts. Thus a new test that shows more branches can make the next check stop for a correct change. A small covered loss can also add up over changes. Seven ledger entries have a range. The later change `harden-gate-ledger` will correct this.
- `range-totals`: The total counts of a file with unstable coverage can change between runs. Thus, while each not-covered count is in its range, the gate does not compare the total counts or the covered counts of the entry. The ratchet command then keeps the old total counts. In such a file, a change can add covered branches and remove the coverage of other branches. The spec adversary checks each changed test for these files.
