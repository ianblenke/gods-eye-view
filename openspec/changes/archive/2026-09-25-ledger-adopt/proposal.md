...
The other 10 errors have other codes. The four types and the error `LEDGER-NO-BASELINE` name 656 files.
...
- Add rule 21 to `AGENTS.md`. It says that a person or an agent must use `adopt` only for code that a merge commit brought from the upstream project. It also says that the person who merges the change must check that the upstream remote has the merged commit. That person must record the result in `review.md`.
...
- Keep the rules for a file that the upstream project did not change. Its gaps cannot open, and the command adopts none of them.
...
- This change does not run `adopt`. The change `upstream-sync` runs it with the merged commit. That change is the first user of the command.

## Known limits and later changes

- `adopt-hand-merged-files`: The command adopts the whole measured gap of a file that the upstream project changed and that a person merged by hand. A gap that a person added when that person resolved the conflicts is inside that count. The person who merges the change reads the Git diff between the merged commit and HEAD for these files.
- `adopt-by-hand`: A person can write an adopt line by hand. The gate checks with Git the merged commit that the line names, and the file that the line names. It does not compare the counts of the line with a measurement. The person who merges the change reads each adopt line.
- `adopt-own-merge-commit`: The gate cannot find the difference between a merged commit of the upstream branch and a merged commit of a local branch. A person can merge a local branch and adopt from the tip of that branch, which is a merged commit. Then the command writes valid adopt lines for the gaps of that branch. Rule 21 of `AGENTS.md` says that a person must not do this. The person who merges a change that uses `adopt` must check that the upstream remote has the merged commit. That person must record the result in `review.md`.
...
- `adopt-untraced-total`: The adopted count of untraced tests is one number for a file: the number of untraced tests of its entry. An adopt line does not keep the names, because the names of about 1800 untraced tests are too many for a person to read.
...
- `adopt-from-text`: The gate gives `LEDGER-ADOPT-FROM` to each text in the field `from` that is not the hash of a merged commit. A branch name that points to a merged commit is also such a text. No scenario names the field `from`.
- `adopt-gate-test-tag`: The test that runs the gate with a line that `gap-ledger-095` rejects has the tags of `gap-ledger-096` and `gap-ledger-097`. It does not have the tag of `gap-ledger-095`. The unit test of `gap-ledger-095` does not run the gate. A later change can add the tag, together with a new ratchet.
- `adopt-rejected-line-totals`: Another history line of the same change with the metric `totals` for a file hides the error `LEDGER-TOTALS-NOT-BASE` for that file. An adopt line that `gap-ledger-095` rejects can have this metric too. So the text "the same errors as with no such line" is not true for this input. This exposure is older than this change.
