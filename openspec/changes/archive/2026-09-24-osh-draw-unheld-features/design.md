...
On the owner's server, the feature collection does not have every feature that the location streams name. The walk of the collection completes, so a page cap does not hide the features. They are absent from the collection. So the dropped case happens on this server, and the map shows only some of the fresh positions that the provider serves.
...
- Give a held feature and an unheld feature one entity id and one position. This is true when the location's `foiId` is the `id` of the held feature. Then, when the features getter throws, the layer keeps that id and that position.
...
- **Cap the stream-drawn features, with a flag like `truncated`.** Rejected. The count is already bounded twice. The freshness gate lets only a location that is not older than the age limit draw. Each stream's page holds at most `OSH_LATEST_LIMIT` records, so the candidate count times that limit is a hard ceiling. A cap would also need an order rule, and the location list has no order across streams.
...
- **Place the host system from the location.** Rejected. One system hosts many features, so the host's marker would sit on one of them and claim to be the system. Mutant M16 exists to stop exactly this, and a test still fails when it runs under this change.
...
This change handles the second cause with one rule, not with a branch. Take one fresh location whose `foiId` is the `id` of a feature. When the layer holds that feature, the location moves it. When the layer does not, the location draws it. Both give the same entity id `osh-foi:<id>` and the same position.
...
Take a location whose `foiId` is the `id` of the held feature. Only these differ: the label, the entity properties, the name in the detail, the host of a click and `placed.streamFeatures`. The layer keeps no map of stream-drawn features across refreshes. So a failed read changes the label, the entity properties and one count for one refresh. The next successful read restores them, and the set of entity ids does not change. `osh-060` pins this with four refreshes.

The seventh option in D52 skips the draw when `partial` is true. Under it, a failed features read removes every feature entity for that refresh, as it does before this change. That happens though the layer holds fresh positions for many of them. It also adds a branch, a test and a mutation for no gain. `partial:true` already tells the viewer that a getter threw.
...
The test that fails when mutant M16 runs keeps its name and every assertion. It still asserts that a fresh location that names an unheld feature never places that feature's host as a fallback. The layer extends the test `[osh-057]` a fresh location that names a feature moves the feature entity and never places a system. That test keeps its name and its tag. It also asserts no system entity and `placed.stream` at zero for a location that names an unheld feature. The host-fallback mutation must make both tests fail.
...
- **A feature can draw twice.** One location names it by id, another by uid alone. Accepted and named as `osh-feature-key-split`. 
