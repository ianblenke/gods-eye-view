...
- [x] 2.4 Change the tests for `credential-boundary-014` to the provider `createHttpGeospatialProvider` in `src/search/reverseGeocodeRoute.test.mjs`.
...
- [x] 2.5 Keep the tests for `osh-028`, `osh-029`, `osh-030`, `osh-031`, `osh-032`, `osh-046`.
- [x] 2.6 Keep the tests for `osh-049`, `osh-057`, `osh-059`, `osh-086`, `osh-087`, `osh-088`.
- [x] 2.7 Keep the tests for `osh-089`, `osh-093`, `credential-boundary-013`, `credential-boundary-015`.

## 3. The code

- [x] 3.1 Merge the upstream branch and resolve the conflicts.
- [x] 3.2 Move the OSH panel to the template, and give the OSH layer the token `3`.
- [x] 3.3 Write `src/app/layers/osh.js` and place the layer in the catalog until 2.3 passes.
- [x] 3.4 Write the server route calls in `src/search/http.js` and `src/search/defaults.js` until 2.4 passes.
- [x] 3.5 Clear the timer in the helper `within` of `src/sdr/controller.test.mjs`.
- [x] 3.6 Raise the ceiling of the ingestion test in `server/providers/transitHistory.test.mjs` to ten seconds.
