...
The two tests of this change start with `passes(root, ['init'])`. On Node 24.14.0 the gates report `COVERAGE-NO-TEST-CONTEXT`, and `init` stops with status 1, so the tests fail. One more test in the same file has the tag `[coverage-gate-049]`. Its title is "stops for a real child process that leaves a live timer, with no stub". It already has the option.
...
- Add a scenario. The requirement of `coverage-gate` already covers the gate report of a live timer.
