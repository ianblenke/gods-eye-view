...
- Follow a next-page link only when it names a later page of the same request. The link must keep the path, and carry no query key outside a fixed query-key allowlist. Stop the walk, with no error, on any other link.
...
- `osh-page-link-allowlist`: The walk follows a next link only when the link keeps the origin, keeps the path, and carries no username and no password. It must also carry no fragment, and use only the query keys `limit`, `offset`, `cursor`, `page`, `startIndex` and `f`. A server that gives its next pages with another key, such as an echoed `bbox` or `datetime` filter, stops the walk. A server whose next link ends the path with a slash also stops the walk. The provider then answers with the pages it already has, and writes no error and no log line. A later change can add a log line or a wider key list, each with a test.
- `osh-page-link-rewrite`: The rebuild writes the value of each accepted key again. The value keeps its meaning, but not always its bytes: `a%20b` becomes `a+b`, and `10;offset=10` becomes `10%3Boffset%3D10`. A server can split a query on `;`, or it can read the bytes of its query string. Such a server gets a request that does not match the link it gave. The same rebuild stops a hidden second key, so the two effects have one cause.
