...
Revert each mutation of this file after its check.

- [x] 3.1 Write the scan test of `credential-boundary-006` for the credential names that the server reads.
  - Rules out: a credential name that the server reads, with no example line and no registry entry.
  - Mutation 1: add `process.env.GEV_PROBE_SECRET` to a server file. The test must name it.
  - Mutation 2: add a destructured, a bracket and a quoted read of it. The test must fail for each.
  - Mutation 3: make the scan patterns match nothing. The positive control must fail.
- [x] 3.2 Add the three missing commented names to `.env.example`, until 3.1 passes.
- [x] 3.3 Write the equality test of `credential-boundary-005` for the registry and the `define` names.
  - Rules out: a client-exposed flag with no matching `define` entry, or the reverse.
...
- [x] 4.1 Write `bundleCredentials.test.mjs` with `credential-boundary-001`, `002` and `016`.
  - Rules out: an unprefixed `VITE_` value in the bundle, a third `define` name, and a scan of the wrong directory.
...
- [x] 4.3 Export `standalonePlugins()` from `server/standalone/vite.config.js`, and build the fixture with it.
- [x] 4.4 Add the tag `credential-boundary-003` to the existing `define` tests.
- [x] 4.5 Add the `Object.keys` equality assertion to each of those tests.
  - Mutation: add a third `define` key. The equality assertion must fail.
- [x] 4.6 Write the key-shaped literal scan test of `credential-boundary-004`.
  - Rules out: a hardcoded key in `src/`, in any file that is not a test file, and a pattern that matches nothing.
...
- [x] 5.1 Write `googleGeocodeProxy.test.mjs` with `credential-boundary-007`, `008` and `009`.
  - Rules out: a route with no key check, the browser key when a server key exists, and a key in a response.
...
- [x] 5.2 Write the projection tests of `credential-boundary-012`.
  - Rules out: a projection that keeps an extra field, or a list longer than its maximum length.
...
- [x] 5.4 Write the tests of `credential-boundary-010` and `011`.
  - Rules out: both modes at once, a blank address, a key in an error text, and no limiter check.
...
- [x] 6.1 Write the tests of `credential-boundary-013` in `placeSearch.test.mjs`.
  - Rules out: a request function that sends `key=`, and an HTTP error answer that closes the search.
...
- [x] 6.4 In each other test that mocks the direct Google host, mock `/api/google/geocode`.
...
- [x] 6.5 Add the test hook and the tests of `credential-boundary-014` to `gevActions.js` and its tests.
  - Rules out: a second request after a `configured:false` answer, and a key in the URL.
...
- [x] 6.7 Write the direct-call scan test of `credential-boundary-015`.
  - Rules out: a direct call to the geocoding host of Google, also in a comment or a data file.
...
- [x] 7.1 Update `.env.example`, `SECURITY.md`, `pinokio/_ENVIRONMENT`, `google-key.js` and `docs/CURRENT-STATE.md`.
  - State the narrower use of the browser key, the wider use of the server key and the OpenAI client secret.
...
- [ ] 9.5 Run each mutation of this file on the final tree, in one session.
...
| Credential registry | `credential-boundary-006` | ADDED | each credential name that the server reads is documented; the positive control |
...
| Google geocoding proxy | `credential-boundary-010` | ADDED | each `400` case, and the `405` case, with zero upstream calls |
