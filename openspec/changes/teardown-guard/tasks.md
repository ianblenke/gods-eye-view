...
- [x] 6.4 Run the spec-adversary and STE-adversary review. Correct the findings. Record the result in `review.md`.
  - `src/data/oshLayer.test.mjs` now has 77 tests. 75 have the `t.after()` teardown guard. One test throws during layer creation. One creates a layer but never calls `init()` on it. Neither needs the guard, matching design.md's stated exception.
  - Task 6.3's undercounting bug recurred several more times during this review's own ratchet runs. `src/scopeMask.test.mjs`, `src/cockpitMarkup.test.mjs` (twice), `src/data/flights.test.mjs`, `src/data/aisLiveVessels.test.mjs`, `src/data/aisWatchdog.test.mjs`, `src/data/cctvCards.test.mjs` and `src/data/cctvLod.test.mjs` all needed the same regular-expression fix as task 6.3. None is a file this change edits.
  - A new manifestation of the same bug: `openspec/trace/links.json`'s own tagged-scenario registry lost 4 of `osh-032`'s 27 real linked tests. Restored the same way, by extracting every real `[osh-032]`-tagged name from source. Several other `TRACE-UNVERIFIED`/`LEDGER-STALE` errors self-corrected on a bare retry, with no code change.
  - Four review rounds ran. Round 1 found 6 spec-adversary findings and 44 ste-adversary findings. Round 2 found 5 and 7. Round 3 found 4 and 4.
  - Round 4, scoped to confirming round 3's fixes, found one residual imprecision. It was corrected in the same round. Every major and critical finding was fixed. A fix that touched the coverage-gate machinery itself was also mutation-proven.
  - Two minor ste-adversary findings, S20 and S21, are accepted by name in `review.md` instead of fixed. Both ask that a single task with two instructions become two numbered tasks. `tasks.md` has no established convention for inserted sub-task numbers, and every affected task id is cross-referenced elsewhere by number. The round-limit rule permits an accepted minor finding from round three onward.
  - `Verdict: PASS` from both reviewers. `review.md` records the full findings list and the `Reviewed-Tree` hash.
