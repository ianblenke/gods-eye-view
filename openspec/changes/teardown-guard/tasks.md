...
- [x] 3.2 Add the destroy-and-cleanup hook after the harness line in each of the four tests.
  - Remove the trailing `destroy` and `cleanup` pair in the three tests that end with it.
...
- [x] 3.4 Prove the guard with a temporary failure in the test at line 467, then revert it.
  ...
  - The run exits with status 1 and prints its plan line. Report the test that reddened.
...
- [x] 4.1 Repeat the probe in the gate image before any gate code changes.
  ...
  - The test guard writes its checked files and its assertion counts from the same exit event. If the marker is missing, the option skips that event.
  - A missing marker means the option would stop coverage measurement for every file while each run stays green.
  ...
  - The plan is severable at one seam.
  ...
  - The run can exit, with the handler running, while the resource list still lacks the timer.
...
- [x] 4.4 Mutation: remove `--test-force-exit` from the main run args.
  - The `[coverage-gate-048]` test reddens on the null status. Put it back. Report the test that reddened.
...
- [x] 5.1 Write the `[coverage-gate-049]` test in `src/tooling/spec/testGuard.test.mjs`.
  ...
  - Assert `results().leaks` names that one file and keeps only the timer and the immediate, dropping the pipe.
...
- [x] 5.5 Add the guard's new input, its new output field, and its exit-event read, until 5.1 and 5.2 are green.
...
- [x] 5.7 Mutation: remove the `Timeout` filter, so `results().leaks` keeps every resource.
