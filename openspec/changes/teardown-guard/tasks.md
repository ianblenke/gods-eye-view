...
- [ ] 1.1 Write `specs/coverage-gate/spec.md` with one ADDED requirement and the scenarios `coverage-gate-048`, `coverage-gate-049` and `coverage-gate-050`.
  - No requirement is MODIFIED. Quote no base text.
  - Check `openspec/trace/retired-ids.json` for the three IDs before you use them.
- [ ] 1.2 Write `proposal.md`: the defect, the two halves, the impact, the limits.
- [ ] 1.3 Write `design.md` with D1 to D6 and the file list.
- [ ] 1.4 Write `tasks.md` from this list.
...
- [ ] 2.1 Add `(t)` to each test signature that has no parameter and makes a layer. Sixty signatures.
  - Rules out: a test that names `t` inside `t.after()` with no `t` parameter, which throws a `ReferenceError` on every run.
- [ ] 2.2 Add `t.after(() => layer.destroy(viewer));` after the `fakeViewer()` line and before `layer.init(viewer)` in each of the 64 tests that make a layer.
...
- [ ] 2.3 Run `node --test src/data/oshLayer.test.mjs`. Confirm 66 tests pass and no test is skipped.
- [ ] 2.4 Prove the guard with a temporary failure, then revert it.
...
- [ ] 2.5 Confirm the assertion error is unchanged. In the transcript of 2.4, the failure names the original message text and the `AssertionError` code.
...
- [ ] 3.1 Add `(t)` to the four tests with an unguarded `env.layer.destroy(env.viewer)`. On this branch the calls are at lines 366, 463, 472 and 690.
- [ ] 3.2 Add `t.after(() => { env.layer.destroy(env.viewer); env.cleanup(); });` after the harness line in each of the four. Remove the trailing `destroy` and `cleanup` pair in the three tests that end with it.
...
- [ ] 3.3 Run `node --test src/data/localGeojson.test.mjs`. Confirm each test passes.
- [ ] 3.4 Prove the guard with a temporary failure in the test at line 467, then revert it. Change `assert.ok(entities.length > 0, ...)` to `assert.ok(entities.length === 0, ...)`.
...
- [ ] 3.5 Confirm no other suite changes. The design's D3 lists each site the facts table counted and why it stays.

## 4. The gate exits each run: `coverage-gate-048`

- [ ] 4.1 Repeat the probe in the gate image before any gate code changes. Write a synthetic test file in a temporary folder that arms `setInterval` and then fails `assert.equal`. Give it a `process.on('exit')` handler that writes a marker file.
...
- [ ] 4.2 Write the `[coverage-gate-048]` test in `src/tooling/spec/gates.test.mjs`.
...
- [ ] 4.3 Add `--test-force-exit` to the main run and to each allocation run in `buildTestRuns()` until 4.2 is green.
- [ ] 4.4 Mutation: remove `--test-force-exit` from the main run args. The `[coverage-gate-048]` test reddens on the null status. Put it back. Report the test that reddened.

## 5. The guard records a live timer: `coverage-gate-049` and `coverage-gate-050`

- [ ] 5.1 Write the `[coverage-gate-049]` test in `src/tooling/spec/testGuard.test.mjs`.
...
- [ ] 5.2 Write the `[coverage-gate-050]` test in the same file. With `getActiveResources: () => ['PipeWrap']`, assert `results().leaks` is `[]`.
  - Change the `deepEqual` on the full `results()` shape to include `leaks: []`.
  - Rules out: a guard that records a leak for a pipe.
- [ ] 5.3 Write the `[coverage-gate-049]` gate test in `src/tooling/spec/gates.test.mjs`. Write a guard file with one `leaks` entry into the output folder of a stubbed run. Assert the gate output has `ERROR GATES-TEST-LEAK` with the test file, and no untrue-coverage entry for any code file.
  - Rules out: a gate that reads `leaks` as a violation.
- [ ] 5.4 Write the `[coverage-gate-050]` gate test in the same file. With a guard file with `leaks: []`, assert no `GATES-TEST-LEAK` error.
- [ ] 5.5 Add the `getActiveResources` input to `createGuard()`, the `leaks` list to `results()`, and the read at the `exit` event in `installGuard()`, until 5.1 and 5.2 are green. Name the test file from `process.argv[1]`, relative to the root.
- [ ] 5.6 Add the `GATES-TEST-LEAK` error to `measure()` until 5.3 and 5.4 are green.
- [ ] 5.7 Mutation: remove the `Timeout` filter, so `results().leaks` keeps every resource. The `[coverage-gate-050]` guard test reddens. Put it back. Report the test that reddened.
- [ ] 5.8 Mutation: make `measure()` skip the `leaks` read. The `[coverage-gate-049]` gate test reddens. Put it back. Report the test that reddened.
