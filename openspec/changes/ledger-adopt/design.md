...
The gates run on the merged tree with the base `origin/main`. They stop the build with 823 errors. 813 of these errors have four kinds:
...
The other 10 errors have other codes: four `COVERAGE-FAKE`, two `TRACE-FAILED-TEST` for one test, and one each of `GATES-TEST-LEAK`, `LEDGER-NO-BASELINE`, `LEDGER-STALE` and `TRACE-LINKS-STALE`. The four kinds and `LEDGER-NO-BASELINE` name 656 files. 16 of these files have content that is not equal to its content in the merged commit, because a person merged them by hand. The other 640 files have the content of the merged commit.
...
The command `adopt` makes one decision for the whole merge commit, and it records that decision as one history line for each file. The lines give the reviewer a list of files, with the counts, and one merged commit. The reviewer can compare the list with the Git diff between the merge base and the merged commit.
...
- A line with the wrong kind, the wrong change name, no file, no full hash of a commit or a wrong count gives no error. It gives no adopted count, and the gate shows the errors of the gap as if the line was not there.
- The gate does not check the head commit and the date of a line.
...
The ledger keeps a count for each test name. An adopt line does not keep the names. The history file then holds about 1800 tests with their names, and a reviewer cannot read them. The line has one number for the file: the number of untraced tests of its entry.

`compareWithBase` adds the counts of all names of the entry. It allows the entry when that number is at or below the adopted count. If that number is above the adopted count, the gate keeps the old error. That error is for each name that is new or that has a higher count than in the base entry.
...
- R2 Give each adopt line the names of the untraced tests. The history file then holds about 1800 tests with their names, and a reviewer cannot read them. A number is enough, because the gate then still stops for a sum above it.
