...
- [x] 2.1 Write the test for `resolveCommit` in `git.test.mjs`, with the tags `gap-ledger-090`.
...
- [x] 2.3 Write the test for `changedByCommit` with a file that only the base side changed, with the tag `gap-ledger-097`.
...
- [x] 4.4 Update the JSDoc of `compareWithBase` and write the JSDoc of the new functions.
