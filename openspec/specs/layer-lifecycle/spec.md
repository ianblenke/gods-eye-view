# layer-lifecycle Specification

## Purpose
Keep the layers of the data manager in a known state when a layer fails. Keep a layer in the manager when its destroy step fails. Send each activity event to each listener, even after a listener fails.
## Requirements
