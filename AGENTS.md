...
21. Use the ledger command `adopt` only for code that a merge commit brought from the upstream project. Do not use it for code that a person wrote in this project. The person who merges a change that uses `adopt` compares the second parent of the merge commit in that change with the upstream remote. That person checks that the upstream remote has that commit, and records the result in `review.md`.
